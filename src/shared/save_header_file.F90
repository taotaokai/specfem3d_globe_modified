...
! ****************************************************************************************************
  ...
  use shared_parameters, only: TOPOGRAPHY, &
    ...
    UNDO_ATTENUATION,MEMORY_INSTALLED_PER_CORE_IN_GB,PERCENT_OF_MEM_TO_USE_PER_CORE, &
...
!          if they prefer having a faster computational time (and use a higher number of cores).
  ...
  if (UNDO_ATTENUATION) then
    ! if (PRINT_INFO_TO_SCREEN) then
    !!KTAO write out info for UNDO_ATTENUATION
    write(IMAIN,*) '*******************************************************************************'
    ...
    write(IMAIN,*) 'ALL dumpings on the disk will require storing ', &
                   disk_size_of_each_dumping*number_of_dumpings_to_do,' GB per core'
...
! Dimitri Komatitsch and Zhinan Xie, CNRS Marseille, France, June 2013.
  ...
  use constants, only: NGLLX,NGLLY,NGLLZ,NDIM,N_SLS,CUSTOM_REAL, &
    IREGION_CRUST_MANTLE,IREGION_INNER_CORE,IREGION_OUTER_CORE !, NTSTEP_BETWEEN_COMPUTE_KERNELS !KTAO remove

  use shared_parameters, only: NTSTEP_BETWEEN_COMPUTE_KERNELS !KTAO add
